...
import {
  ...
  export default function Category() {
    ...
    const deleteCategory = async(_id) => {
        try {
            const deleteCat = await axios.put("/api/users/category_delete",{
                _id: _id,
            })
            toast({
                title: "Deleted Card",
    ...
    const PostHistory = async() => {
        try {
            const upload = await axios.post("/api/users/category",{
                description: descriptionNew,
                photo_url: url,
                title: titleNew,
            })
            toast({
                title: "category Uploaded to category wall",
                description: "Refresh page to see",
            })
            settitleNew("");
            setdescriptionNew("");
            ...
            console.log("could not add add category", error);
        }
    }
    useEffect(() => {
        const getContent = async() => {
            try {
                const category = await axios.get("/api/users/category");
                //SetImgUrl(category.data.Post.photo_url[0]);
                setHistories(category.data.Post);
                //console.log(category.data.Post);
            ...
            {loading ? (<center><div className='p-8 m-8'><Spinner label="Loading..." color="success" size='lg' className='m-8 p-8'/></div></center> ) : (
                <div className='flex flex-row flex-wrap'>
                    {Histories.map((hist, key)=>(
                        ...
                        {isadmin && 
                        <Button size='sm' className='text-red-500 text-bold m-2 aline-end' onClick={()=>deleteCategory(hist._id)}><div className='flex inline-center'><p className='text-black'>DELETE  </p></div></Button>
                        }
                        </Card>
