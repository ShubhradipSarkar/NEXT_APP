...
export default function UserProfile({params}) {
    ...
    const updateContent = async() => {
        try {
            const update = await axios.put("/api/users/category",{
                _id: Id,
                title: title,
                description: text,
            })
    ...
    useEffect(() => {
        const getall = async() => {
            try {
                const dt = await axios.put("/api/users/category_each",{
                    _id: params_uri.id
                })
                console.log(dt);
    ...
    return (
        ...
        <div className='min-h-screen'>
            ...
            <center><div><Textarea value={text} onChange={(e)=>{settext(e.target.value)}} className='h-full 2xl:w-full md:w-80 p-3'/> <Button onClick={updateContent} size='sm' className='m-2'>Update Content </Button></div></center>
            </div>
            }
            <Footer_/>
        </div>
