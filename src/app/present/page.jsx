...
import {
  ...
  export default function present({
    ...
    const PostHistory = async() => {
        try {
            const upload = await axios.post("/api/users/present",{
                description: descriptionNew,
                photo_url: url,
                title: titleNew,
            })
            toast({
                title: "present Uploaded to present wall",
            ...
            console.log("could not add add present", error);
        }
    }
    const deleteHistory = async(_id) => {
        try {
            const deleteCat = await axios.put("/api/users/present_delete",{
                _id: _id,
            })
            toast({
                title: "Deleted Card",
    ...
    useEffect(() => {
        const getContent = async() => {
            try {
                const present = await axios.get("/api/users/present");
                //SetImgUrl(present.data.Post.photo_url[0]);
                setHistories(present.data.Post);
                //console.log(present.data.Post);
    ...
    return (
        ...
        <div>
            ...
            <center><p className='text-3xl m-1 mt-2 italic'>আমাদের নিয়মিত কর্মসূচি সমূহ</p></center>
            <div className=' flex flex-row items-center justify-center flex-wrap min-h-screen m-3'>
            {loading ? (
